 ...
 *
 * FIXED (FEAT_SPAWN_BUDGET): Returns 0 to activate the per-region
 * budget once resbank.mul coordinates are remapped to match the 9
 * regions in resregions.txt.
 */
static int
 ...
 *
 * Queues a respawn for templateIndex in the region covering loc when
 * newPriority (amount * 58) exceeds currentPriority (respawnAmount *
 * respawnCountdown). InitRespawn decrements respawnCountdown each cycle
 * and clears the slot when it hits zero.
 *
 * FEAT_SPAWN_BUDGET: accumulate amounts instead of overwriting and only
 * start the countdown on the first death; InitRespawn returns the
 * accumulated budget via AddToQuantity when the countdown expires.
 */
void
CResBankManager_ScheduleRespawnForTemplate(CLocation *loc, int templateIndex, int16_t amount, int16_t templateValue)
{
	...
	if (feat(FEAT_SPAWN_BUDGET)) {
		// Accumulate: multiple NPC deaths of the same resource type
		// add up instead of overwriting. Only start countdown if not
	...
	region->respawnAmount[templateIndex] = (uint16_t)amount;
	region->respawnCountdown[templateIndex] = 0x3A; // 58 cycles countdown
}
 ...
 *
 * FEAT_SPAWN_BUDGET: when a countdown expires, returns the accumulated
...
CResBankRegion_SpawnAtPointInBox(CResBankRegion *region, uint16_t templateId, SubRegion *sub)
{
	...
	if (result == NULL) {
		g_SpawnCreateFailCount++;
	} else if (feat(FEAT_SPAWN_BUDGET)) {
		NPCTemplate *tmpl = CResManager_GetTemplateByID(templateId);
		if (tmpl != NULL) {
			CResourceNode *nd;
			for (nd = tmpl->resourceNodes; nd != NULL; nd = nd->next) {
 ...
 *
 * Picks a random sub-region from entry, caps density via the scalingWts
 * or the area-based formula, counts existing mobiles of templateId (and
 * its paired id offset 1000), and spawns a new mob at a random point
 * inside the box (or at the midpoint when noWander is set).
 *
 * FIXED - Binary bug: area formula overrides regionlimit. The binary
 * unconditionally sets count = area/2560, ignoring the per-template
 * regionlimit (scalingWts). Use area/2560 only as a fallback when
 * scalingWts is 0.
 *
 * FIXED - Binary bug: shopkeeper accumulation in small sub-regions.
 * For small non-regionlimit boxes the density check is skipped, so
 * noWander shops (excluded from DEFCON) accumulate forever. When
 * noWander is set and count is still 0 after the area formula, force
 * count=1.
 *
 ...
 * sub-regions instead of only the chosen box.
 */
static CItem *
CResBankRegion_SpawnInSubRegion(CResBankRegion *region, ResSpawnEntry *entry, int noWander)
{
	...
	if (strcmp(sub->name, "SCALING") != 0) {
		// FIXED: only use area formula when no regionlimit is set.
		if (count == 0) {
			int dx, dy, area, areaCount;
			dx = (int)sub->x2 - (int)sub->x1;
			dy = (int)sub->y2 - (int)sub->y1;
			area = dx * dy;
			areaCount = area / 0xA00;
			if (areaCount > 0)
				count = areaCount;
			// FIXED: shopkeeper accumulation fix for small
			// sub-regions without regionlimit.
			else if (noWander)
				count = 1;
		}
	}
	...
	if (count != 0) {
		...
		// matching the binary.
		if (noWander && ((int)sub->x2 - (int)sub->x1) * ((int)sub->y2 - (int)sub->y1) < 0xA00) {
			int si;
 ...
 *
 * Returns 1 if the template is a shopkeeper and the region forbids
 * wandering - i.e. the NPC must stay at its spawn spot.
 */
static int
CResBankRegion_GetExactPosition(CResBankRegion *region, NPCTemplate *tmpl)
{
	if (tmpl->type == 4 && region->noWander == 1)
		return 1;
	return 0;
}
